...
function formatCell(str: string | number, width: number): string {
...
export function formatHuman(report: TokenDiffReport): string {
  ...
  lines.push(
    pc.bold(
      `${formatCell('Target', colTarget)} ${formatCell('Tokens', colTokens)} ${formatCell('Chars', colChars)} ${formatCell('Lines', colLines)}`
    )
  );
  ...
  // Data rows
  lines.push(
    `${formatCell(report.before.label, colTarget)} ${formatCell(report.before.token_count, colTokens)} ${formatCell(report.before.char_count, colChars)} ${formatCell(report.before.line_count, colLines)}`
  );
  lines.push(
    `${formatCell(report.after.label, colTarget)} ${formatCell(report.after.token_count, colTokens)} ${formatCell(report.after.char_count, colChars)} ${formatCell(report.after.line_count, colLines)}`
  );
  lines.push(pc.dim('-'.repeat(colTarget + colTokens + colChars + colLines + 3)));
  ...
  lines.push(
    `${pc.bold(formatCell('Diff', colTarget))} ${paddedTokenDiff} ${paddedCharDiff} ${paddedLineDiff}`
  );
  lines.push('');
